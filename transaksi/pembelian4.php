...
<body id="page-top">
    ...
    <script>
            function updateQty(checkbox) {
                var total = 0;
                var checkboxes = document.getElementsByClassName('selectProduct');
                for (var i = 0; i < checkboxes.length; i++) {
                    if (checkboxes[i].checked) {
                        var row = checkboxes[i].parentNode.parentNode;
                        var price = parseFloat(row.getElementsByTagName('td')[1].innerText.replace('Rp. ', '').replace(',', ''));
                        total += price;
                    }
                }
            ...
            function hitungSisa() {
                var totalBayar = parseFloat(document.getElementById('totalbarang').innerText);
                var bayar = parseFloat(document.getElementById('bayar').value);
                var sisaBayar = totalBayar - bayar;
                document.getElementById('sisaBayar').innerText = sisaBayar;
            }
